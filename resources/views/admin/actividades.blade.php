...
<head>
  ...
  <meta name="description" content="Sección administrativa de actividades">
...
<main role="main">
	<div class="container">
    <a href='<?= route("admin.actividadNew"); ?>' class='btn btn-success m-b-1em float-right my-4'><i class='fas fa-plus'></i> Crear actividad</a>
    <h1 class="display-4 mt-2">Lista de Actividades</h1>
    <hr>
    <!-- MODAL -->
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title" id="exampleModalLabel"> Eliminar Actividad</h4>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
      
      <form action="<?= route("admin.actividadDelete"); ?>" method="POST">
        @method('DELETE')
        @csrf
    ...
    <div class='table-responsive'>
        ...
        <tr>
          <td scope='row'><?= $act->id ?></td>
					<td><a href='https://drive.google.com/open?id=<?=$act->id_drive?>' target='_blank'><?= $act->nombre ?></a></td>
					<td><?= $act->plano_linguistico ?></td>
				  <td><?= $act->rango_edad ?></td>
					<td>
						<!--EDIT-->
            <a href='<?= route("admin.actividadEdit" , $act->id); ?>' class='btn btn-primary m-1'><i class='fas fa-edit'></i> Edit</a>
						<!--DELETE-->
            <button type='button' class='btn btn-danger deletebtn m-1' data-toggle='modal' data-target='#deleteModal' data-actividad='<?= $act->id; ?>'><i class='fas fa-trash-alt'></i> Delete</button>
					</td>
        </tr>
        <?php 
          endforeach;
